...
proc maskWord w {
  ...
  ###etm: set y [expr $Y($w) + $CharHeight / 2]
  ###etm: height of the dashes should be lower for Japanese:
  set y [expr $Y($w) + $CharHeight * $MaskHeight]
...
#etm: removed right parenthesis that would be shown after $label:
#     .c create text [expr [winfo width .c] / 2 + $xshift] \
#	[expr [winfo height .c] / 2 + 2 * $MultiChoiceVGap + int($i / 2) * $MultiChoiceVGap] \
#	-text "$label) [string map {_ { }} $choice]" -anchor w -fill $TextColor -font $font
...
proc presentQuestion {question item font justify} {
  ...
  giveFeedback $code
  if {$ResultFile != {} && $item != {}} {
    ###etm  puts $ResultFile "$Subject [split $item :] ?$tag $Answer $code $delay"
    puts $ResultFile "$Subject [split $item :] ?$tag $Answer $code $delay $question"
    flush $ResultFile
  }
  return $code
}
...
proc selfPacedPresentSentence item {
  global Tag NumWords GoKey SkipKey StopTime ResultFile Subject Abort Word \
      ClockRes TextColor MaskColor ShowWords WindowStep
...
No data will be recorded.}
  ...
  if {$Continuing} {
    #etm:    set ResultFile [openFile Results/$Subject.dat "a"]
    set ResultFile [openFile Results/$Exp-$Subject.dat "a"]
  } else {
    #etm:    set ResultFile [openFile Results/$Subject.dat "w"]  
    #    includes exp and subj in the file name, and 
    #    always **APPENDS** to make sure I dont lose data by mistake:
    set ResultFile [openFile Results/$Exp-$Subject.dat "a"]

    # Store the items for later continuation:
    #etm:    set ExpFile [openFile Results/$Subject.itm "w"]
    set ExpFile [openFile Results/$Exp-$Subject.itm "w"]
